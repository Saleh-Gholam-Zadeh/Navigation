...
class InputEmbedding(torch.nn.Module):
    ...
    def __init__(self, input_dim, embedding_dim, scale, use_tags=True):
        ...
        self.input_embeddings = torch.nn.Sequential(
            torch.nn.Linear(input_dim, linear_embedding_dim),  # 130-->62
            torch.nn.ReLU(),
        )
    ...
    def start_enc(self, vel):
        ...
        v = torch.zeros(vel.size(-3),vel.size(-2), self.embedding_dim, device=vel.device) #check v
        v[:,:, -2] = 1
        return v # v =[6 64] --> v[0<i<5] = [0 0 0 ... 0 0 1 0]
