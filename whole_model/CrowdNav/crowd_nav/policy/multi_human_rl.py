...
class MultiHumanRL(CADRL):
    ...
    def predict(self, state,joint_xy_history,occ_history):
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            for action in self.action_space:
                ...
                batch_next_states = torch.cat([torch.Tensor([next_self_state + next_human_state]).to(self.device)
                                              for next_human_state in next_human_states], dim=0)    # ans =[n-human,14]
                ...
                # VALUE UPDATE
                if(len(joint_xy_history.size())==3):     #hatman bayad information next ra ezafe konim inja
                    joint_xy_history=joint_xy_history.unsqueeze(0)  #[1 hist-len n-human+1 2(xy)]
                ...
                joint_xy_next = torch.cat([robot_xy_next,human_xy_next],dim=-2) #[ n-human+1, 2]

                joint_xy_current = joint_xy_history[:,-1,:,:].squeeze(0) #ans = [n-human+1 ,2(xy)]


                last_occ    = self.pool(None, joint_xy_current,joint_xy_next).unsqueeze(0) #[1, 1 ,32]


                joint_xy_history_plus=torch.cat([joint_xy_history[:,1:,:,:],joint_xy_next.unsqueeze(0).unsqueeze(0)],dim=-3)  #[batch ,hist-len ,n-human+1,2(xy)]
                occ_history_plus = torch.cat([occ_history[:,1:,:],last_occ],dim=-2)
                representation_env_plus = self.traj_model(joint_xy_history_plus, occ_history_plus)[0].data  # joint_xy=[batch, hist_len, n-human+1,2(xy)]  // occ_history=[100,hist_len-1,32] //ans=[100 128]

                next_state_value = self.model(rotated_batch_input,representation_env_plus).data.item()
        ...
        if self.phase == 'train':
            ...
            self.last_state_unrotated = torch.cat([torch.Tensor([state.self_state + human_state]).to(self.device)  # added by saleh : because now we are in the RL phase and we need to push unrotated_state in the memory
