...
traj_model.load_state_dict(torch.load(os.path.join( os.getcwd() , "directional.pkl.state" ))['state_dict'])
traj_model.eval()
...
def main():
    ...
    policy.configure(policy_config,traj_model)
    ...
    trainer = Trainer(model,traj_model, memory, device, batch_size)
    explorer = Explorer(traj_model,env, robot, device, memory, policy.gamma, target_policy=policy)
    ...
    print('IL has finished')
    time.sleep(2)
    print('RL has started')
    policy.set_env(env)
